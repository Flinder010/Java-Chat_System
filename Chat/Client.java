...
public class Client extends JFrame implements ActionListener{
	...
	private PrintWriter pwNet = null;
	...
	public Client(){
		...
		try{
			...
			new Thread( new ClientThread( sk , jta ) ).start(); // 加入多线程
			String line = null;
			while( true ){
	...
	public void actionPerformed( ActionEvent e){
		...
		if( text.equals( "exit" ) == false ) text = "客户端对服务器说：" + text;
		if( text.equals( "exit" ) == false ) jta.append( text + System.lineSeparator() );
		try{
			bfrw.write( text );
			bfrw.newLine();
			bfrw.flush();
...
class ClientThread implements Runnable{
	...
	public ClientThread( Socket sk , JTextArea jta ){
		this.sk = sk;
		this.jta = jta;
	}
	public void run(){
